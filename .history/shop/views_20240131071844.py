...
def edit_product_view(request, product_id):
    ...
    # Используем Paginator для пагинации списка товаров
    products_list = Product.objects.all()
    paginator = Paginator(products_list, 10)  # Разбиваем список товаров на страницы по 10 товаров на каждой странице
    page = request.GET.get('page')  # Получаем номер текущей страницы из параметра запроса 'page'
    products = paginator.get_page(page)  # Получаем объект страницы товаров

    return render(request, 'edit_product.html', {'products': products, 'form': form, 'product': product})
# Order Views
